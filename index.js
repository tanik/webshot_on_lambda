...
exports.handler = (event, context, callback) => {
  const fontConfigSrcPath = path.join(__dirname, 'fontconfig')
  const fontLibPath = '/tmp/fontconfig/usr/lib/'
  exec(`rm -rf /tmp/fontconfig; cp -r ${fontConfigSrcPath} /tmp/.; /tmp/fontconfig/usr/bin/fc-cache -fs;`, (error, stdout, stderr) => {
    if(error){
      console.error(`font setting error: ${error}, stdout: ${stdout}, stderr: ${stderr}`)
    }
    const scriptPath = path.join(__dirname, "webshot.js")
    const phantomCmd = `LD_LIBRARY_PATH=${fontLibPath} ${phantom} ${scriptPath} ${event.url}`
    exec(phantomCmd, (error, stdout, stderr) => {
      if (error) {
